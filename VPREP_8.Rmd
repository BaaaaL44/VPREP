...
title: "`r params$year` `r params$season`i `r params$subj` Érettségi Vizsga Eredményei  (`r params$centrum` `r params$school`)"
author: "Tóth Bálint"
date: "`r Sys.Date()`"
output:
  html_document:
...
## Ez ahhoz kell, hogy az asis engine rendesen működjön, és lehessen feltételesen szövegblokkokat megjeleníteni. Ne piszkáld, így működik. 
library(knitr)
...
## Használt csomagok letöltése. Annyiszor át lett írva a kód, hogy talán van benne olyan, amit nem is hív a kód soha, de nem kér enni, így a 
## biztos. (pl. szerintem a matrixcalc már nem kell)
library(readxl)
library(ggplot2)   
library(car)      
library(huxtable)
...
```{r, warning = FALSE, message = FALSE, echo = FALSE}
## Ez a blokk olvassa be az adatokat az Excel-fájlból, és végzi el az adatelőkészítést egy lépésben. Minden kis kondícionális blokk egy külön tárgyhoz tartozik. Azért kellett így megoldani, mert minden tárgy más számú feladatot tartalmaz, más pontozással, és így volt legkevésbé macerás elérni, hogy a százalékok mindenhol stimmeljenek. Az érettségi követelmények változása esetén csak át kell írni a feladattípusok összpontszáma szerint.

if (params$subj %in% c("Angol", "Német")){

fulldata = read_excel('angol_2023_full.xlsx')
fulldata = fulldata %>% mutate(Olvasott_százalék = (Olvasott/33)*100, Hallott_százalék = (Hallott/33)*100, Írás_százalék = (Írás/33)*100, Nyelv_százalék = (Nyelv/18)*100, Százalék_teljes = (Össz/150*100), Írásbeli_százalék = (Írásbeli/117)*100, Szóbeli_százalék = (Szóbeli/33)*100, Státusz = case_when(Írásbeli_százalék <= 12.5 | Szóbeli_százalék < 12.5 ~ "Érvénytelen", Írásbeli_százalék >= 12.5 & Írásbeli <= 38 ~ "Kettesért szóbelizik", Írásbeli >= 38 ~ "Írásbelin átment"), Jegy = case_when(Százalék_teljes < 24.5 ~ "Elégtelen", Százalék_teljes >= 24.5 & Százalék_teljes < 39.5 ~ "Elégséges", Százalék_teljes >= 39.5 & Százalék_teljes < 59.5 ~ "Közepes", Százalék_teljes >= 59.5 & Százalék_teljes < 79.5 ~ "Jó", Százalék_teljes >= 79.5 ~ "Jeles"))
...
## Ez csinál egy egyszerű oszlopdiagramot a tanulók számáról osztályonként, beágyazza egy ggplotly függvénybe, hogy interaktív legyen.
...
## Az előző grafikon tartalmát jeleníti meg táblázatos formában, rendezés és szűrés után, százalékos formában, 2 tizedesjegyre kerekítve.
...
## Tanárok szerint rajzol egy oszlopdiagramot a gyakoriságokról, ugyanígy egy interaktív ggplotly wrapperbe ágyazva
...
```{r warning = FALSE, message = FALSE, echo = FALSE, eval = TRUE}
## A fenti grafikon tartalmát jeleníti meg táblázatos formában, százalékokban kifejezve
...
stats = stats %>% mutate(Változó = str_replace_all(stats$Változó, "_", " "))
...
ggplotly(ggplot(data = best, aes(x = reorder(Név,Százalék_teljes), y = Százalék_teljes, fill = reorder(Név, Százalék_teljes))) +geom_col() + labs(fill = "Tanuló") + theme(axis.title.y=element_blank()) + coord_flip() + facet_wrap(~ Osztály) + ylab("Elért eredmény"), height = 900, width = 900, tooltip = "Százalék_teljes")
```
...
if (sum(!is.na(fulldata$Szóbeli)) > length(fulldata$Szóbeli)/2 | sum(fulldata$Szóbeli) == 0){
...
```{r warning = FALSE, message = FALSE, echo = FALSE, eval = TRUE}
TIPS_rész = c("Tanár", "Számtani átlag", "Medián, a sorba rendezett adatok közül a középső, az adatok 50%-a alacsonyabb ennél az értéknél", "Szórás, az átlagtól való négyzetes eltérések átlagának gyöke", "Medián Abszolút Eltérés, a mediántól való abszolút eltérések mediánja. A szórásnál kevésbé érzékeny a szélsőértékekre", "A legkisebb megfigyelt érték", "A legnagyobb megfigyelt érték")
...
rész_desc = rész_desc %>% mutate(Feladat = str_replace_all(Feladat, "_", " "))
colnames(rész_desc) = text_spec(c("Vizsgarész", "Átlag", "Medián", "Szórás", "MAD", "Min", "Max"), "html", tooltip = TIPS_rész)
kable(rész_desc, "html", escape = F, digits = 3) %>% kable_styling("striped", full_width = F, position = "center")
```

```{r warning = FALSE, message = FALSE, echo = FALSE, eval = ncol(fulldata %>% select(ends_with("százalék"), -Írásbeli_százalék)) > 1}
if(params$subj == "Turisztika"){
box_rep = ggplot(data = fulldata_long, aes(x = str_replace_all(Feladat, "_", " "), y = Százalék, fill = Feladat)) + geom_boxplot(lwd = 1) + coord_flip()
div(ggplotly(box_rep), align = "center")
} else{
box_rep = ggplot(data = fulldata_long, aes(x = str_replace_all(Feladat, "_", " "), y = Százalék, fill = Feladat)) + geom_boxplot(lwd = 1) + xlab("Feladat")
div(ggplotly(box_rep), align = "center")  
}
```
...
rep_pair = fulldata_long %>% mutate(Feladat = str_replace_all(Feladat, "_", " "))%>% pairwise_t_test(Százalék ~ Feladat, paired = TRUE, p.adjust.method = "holm")
rep_pair = rep_pair[,2:ncol(rep_pair)]
rep_pair = rep_pair[,1:ncol(rep_pair)-1]
...
```{r warning = FALSE, message = FALSE, echo = FALSE, eval = params$subj == "Turisztika"}
második_plot = ggplotly(ggplot(fulldata, aes(x = Írásbeli_százalék, y = Szóbeli_százalék, label = Név)) + geom_point() + geom_smooth(method = "lm")) %>% layout(title = paste('Írásbeli és szóbeli eredmények korrelációja', " (r = ",round(cor(fulldata$Írásbeli_százalék, fulldata$Szóbeli_százalék), 3), ")"), plot_bgcolor = "#e5ecf6", xaxis = list(title = "Írásbeli pontszám"), 
yaxis = list(title = "Szóbeli pontszám"), titlefont = list(size = 15))
...
p = plot_ly(data = fulldata, x = ~Olvasott_százalék, y = ~Hallott_százalék, z = ~Írás_százalék, type="scatter3d", mode="markers", color = ~ Nyelv_százalék, text = ~Név, width = 800, height = 800) %>% layout(
    scene = list(aspectmode = "cube", xaxis = list(range = c(0,100), title = 'Olvasott Szövegértés (x)'),yaxis = list(range = c(0, 100), title = 'Hallott Szövegértés (y)'), zaxis = list(range = c(0,100), title = 'Íráskészség (z)')))
...
második_plot = ggplotly(ggplot(második_plot_data, aes(x = Írásbeli_százalék, y = Szóbeli_százalék, label = Név)) + geom_point() + geom_smooth(method = "lm")) %>% layout(title = paste('Írásbeli és szóbeli eredmények korrelációja', " (r = ",round(cor(második_plot_data$Írásbeli_százalék, második_plot_data$Szóbeli_százalék), 3), ")"), plot_bgcolor = "#e5ecf6", xaxis = list(title = "Írásbeli százalék"), 
yaxis = list(title = "Szóbeli százalék"), titlefont = list(size = 15))
...
második_plot_data = as.data.frame(fulldata %>% select(Írásbeli_százalék, Szóbeli_százalék, Név))
lin = lm(data = második_plot_data, Szóbeli_százalék ~ Írásbeli_százalék)
második_plot = ggplotly(ggplot(második_plot_data, aes(x = Írásbeli_százalék, y = Szóbeli_százalék, label = Név)) + geom_point() + geom_smooth(method = "lm")) %>% layout(title = paste('Írásbeli és szóbeli eredmények korrelációja', " (r = ",round(cor(második_plot_data$Írásbeli_százalék, második_plot_data$Szóbeli_százalék), 3), ")"), plot_bgcolor = "#e5ecf6", xaxis = list(title = "Írásbeli pontszám"), 
yaxis = list(title = "Szóbeli pontszám"), titlefont = list(size = 15))
...
osztály_t = t.test(data = fulldata, Százalék_teljes ~ Osztály, pool.sd = FALSE, var.equal = FALSE, conf.level = 0.95)
tidy = tidy(osztály_t)
p_val = tidy[5]
...
TIPS_pair_osztaly = c("Az első vizsgált osztály", "A második vizsgált osztály", "Az első osztály mintaelemszáma", "A második osztály mintaelemszáma", "t-statisztika, a két osztály átlaga közötti különbségnek a két osztály varianciájával való hányadosa", "Szabadsági fok, a t-eloszlás kritikus értékeinek megállapításához szükséges", "p-érték, ha 0.05-nél kisebb, az eredmény statisztikailag szignifikánsnak tekinthető", "Korrigált p-érték", "korrigált p-érték szignifikanciakódja")
pairwise_osztaly = fulldata %>% pairwise_t_test(Százalék_teljes ~ Osztály, pool.sd = FALSE, var.equal = FALSE, paired = FALSE, p.adjust.method = "holm")
pairwise_osztaly = pairwise_osztaly[,2:ncol(pairwise_osztaly)]
colnames(pairwise_osztaly) = text_spec(c("Osztály 1", "Osztály 2", "n1", "n2", "Tesztstatisztika", "df", "p", "p.adj", "Szig. kód"), "html", tooltip = TIPS_pair_osztaly)
...
TIPS_tanar = c("Két tanár átlagának különbsége", "Első vizsgált tanár", "Második vizsgált tanár", "t-statisztika, matematikailag a két tanár átlagának különbsége osztva a két tanár szórásával", "p-érték, ha 0.05-nél kisebb, az eredmény szignifikánsnak tekinthető", "Szabadsági fok, a t-statisztika kritikus értékeinek megállapításához szükséges", "Konfidenciatartomány alsó határa", "Konfidenciatartomány felső határa", "A végzett t-próba típusa. Mivel jelen adatoknál a két csoport varianciájának azonossága nem garantálható, ez Welch-féle t-próba", "Alternatíva: Mivel nincs előzetes hipotézisünk az eltérés irányáról, kétoldalas tesztet alkalmazunk")  
tanar_t = t.test(data = fulldata, Százalék_teljes ~ Tanár, var.equal = FALSE, pool.sd = FALSE,  conf.level = 0.95)
...
```{asis, echo = length(levels(fulldata$Tanár)) > 1}
A`r ifelse(length(levels(fulldata$Tanár)) > 2, "z egyutas varianciaanalízis", " független mintás t-próba")` eredménye `r ifelse(p_val_tanár > 0.05, "nem szignifikáns", "szignifikáns")` (p = `r as.numeric(p_val_tanár)`).
```
...
TIPS_pair_tanar = c("Az első vizsgált osztály", "A második vizsgált osztály", "Az első osztály mintaelemszáma", "A második osztály mintaelemszáma", "t-statisztika, a két osztály átlaga közötti különbségnek a két osztály varianciájával való hányadosa", "Szabadsági fok, a t-eloszlás kritikus értékeinek megállapításához szükséges", "p-érték, ha 0.05-nél kisebb, az eredmény statisztikailag szignifikánsnak tekinthető", "Korrigált p-érték", "korrigált p-érték szignifikanciakódja")
pairwise_tanar = fulldata_long %>% pairwise_t_test(Százalék ~ Tanár, paired = FALSE, pool.sd = FALSE, p.adjust.method = "holm")
pairwise_tanar = pairwise_tanar[,2:ncol(pairwise_tanar)]
colnames(pairwise_tanar) = text_spec(c("Osztály 1", "Osztály 2", "n1", "n2", "Tesztstatisztika", "df", "p", "p.adj", "Szig. kód"), "html", tooltip = TIPS_pair_tanar)
...
```{r warning = FALSE, message = FALSE, echo = FALSE, eval = length(levels(fulldata$Nem)) > 1}
TIPS_nem = c("Nemek átlagának különbsége", "Első vizsgált csoport", "Második vizsgált csoport", "t-statisztika, matematikailag a két osztály átlagának különbsége osztva a két osztály szórásával", "p-érték, ha 0.05-nél kisebb, az eredmény szignifikánsnak tekinthető", "Szabadsági fok, a t-statisztika kritikus értékeinek megállapításához szükséges", "Konfidenciatartomány alsó határa", "Konfidenciatartomány felső határa", "A végzett t-próba típusa. Mivel jelen adatoknál a két csoport varianciájának azonossága nem garantálható, ez Welch-féle t-próba", "Alternatíva: Mivel nincs előzetes hipotézisünk az eltérés irányáról, kétoldalas tesztet alkalmazunk")  
nem_t = t.test(data = fulldata, Százalék_teljes ~ Nem, var.equal = FALSE, pool.sd = FALSE, conf.level = 0.95)
tidyt_n = tidy(nem_t)
...
if (sum(!is.na(fulldata$Szóbeli)) <= 3 | sum(fulldata$Szóbeli == 0)){
...
## Függelék: az összes tanuló eredményei

Alább elérhető a vizsgán résztvevő minden tanuló eredménye táblázatos formában. A helytakarékosság miatt a táblázatnak egyidejűleg csak egy része jelenik meg, a jobb alsó sarokban található gombokkal lapozhatunk, az oszlopok tetején lévő szűrőkkel pedig szűrhetjük a megjeleníteni kívánt tanulókat. Névre kereséshez vagy a táblázat feletti 'Search' mezőt, vagy a 'Név' oszlop feletti mezőt használjuk, utóbbi esetében a beírt szöveg nem mindig jelenik meg a keresőmezőben.

```{r warning = FALSE, message = FALSE, echo = FALSE}
DTdata = fulldata %>% select(Név, Nem, ifelse(length(levels(fulldata$Tanár)) > 1, "Tanár", 0), ifelse(length(levels(fulldata$Osztály)) > 1, "Osztály", 0), ends_with("százalék"), Szóbeli, Százalék_teljes, Státusz, Jegy) %>% mutate_if(is.numeric, round, 2)

datatable(DTdata, rownames = FALSE, filter = "top", colnames = str_replace_all(colnames(DTdata), "_", " "), options = list(serverSide = FALSE))
```
