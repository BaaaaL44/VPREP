...
title: "`r params$subj` Érettségi Vizsga Eredményei"
author: "Tóth Bálint"
date: "`r Sys.Date()`"
output:
  html_document:
...
```{r, warning = FALSE, message = FALSE, echo = FALSE}
## Ez a blokk olvassa be az adatokat az Excel-fájlból, és végzi el az adatelőkészítést egy lépésben. Minden kis kondícionális blokk
## egy külön tárgyhoz tartozik. Azért kellett így megoldani, mert minden tárgy más számú feladatot tartalmaz, más pontozással, és így
## volt legkevésbé macerás elérni, hogy a százalékok mindenhol stimmeljenek.

if (params$subj %in% c("Angol", "Német")){

fulldata = read_excel('angol2.xlsx')
fulldata = fulldata %>% mutate(Olvasott_százalék = (Olvasott/33)*100, Hallott_százalék = (Hallott/33)*100, Írás_százalék = (Írás/33)*100, Nyelv_százalék = (Nyelv/18)*100, Százalék_teljes = (Össz/150*100), Írásbeli_százalék = (Írásbeli/117)*100, Szóbeli_százalék = (Szóbeli/33)*100, Státusz = case_when(Írásbeli_százalék < 12 | Szóbeli_százalék < 12 ~ "Érvénytelen", Írásbeli_százalék >= 12 & Írásbeli < 38 ~ "Kettesért szóbelizik", Írásbeli >= 38 ~ "Írásbelin átment"), Jegy = case_when(Százalék_teljes < 25 ~ "Elégtelen", Százalék_teljes >= 25 & Százalék_teljes < 40 ~ "Elégséges", Százalék_teljes >= 40 & Százalék_teljes < 60 ~ "Közepes", Százalék_teljes >= 60 & Százalék_teljes < 80 ~ "Jó", Százalék_teljes >= 80 ~ "Jeles"),Jegy_raw = case_when(Százalék_teljes < 25 ~ 1, Százalék_teljes >= 25 & Százalék_teljes < 40 ~ 2, Százalék_teljes >= 40 & Százalék_teljes < 60 ~ 3, Százalék_teljes >= 60 & Százalék_teljes < 80 ~ 4, Százalék_teljes >= 80 ~ 5))
...
```{asis, echo = TRUE}
 A vizsgán `r jegy_raw_freq[1,2]` tanuló szerzett elégtelen, `r jegy_raw_freq[2,2]` tanuló elégséges, `r jegy_raw_freq[3,2]` tanuló közepes, `r jegy_raw_freq[4,2]` jó, `r jegy_raw_freq[5,2]` pedig jeles osztályzatot. Az érettségi vizsga jegyben számolt eredményének átlaga `r round(mean(fulldata$Jegy_raw), 2)`
```
...
if(params$subj == "Turisztika"){
box_rep = ggplot(data = fulldata_long, aes(x = str_replace_all(Feladat, "_", " "), y = Százalék, fill = Feladat)) + geom_boxplot(lwd = 1) + coord_flip() + scale_fill_brewer(palette = "BrBG") + theme_dark()
div(ggplotly(box_rep), align = "center")
} else{
box_rep = ggplot(data = fulldata_long, aes(x = str_replace_all(Feladat, "_", " "), y = Százalék, fill = Feladat)) + geom_boxplot(lwd = 1) + xlab("Feladat") + scale_fill_brewer(palette = "BrBG") + theme_dark()
div(ggplotly(box_rep), align = "center")  
}
```
...
```{r warning = FALSE, message = FALSE, echo = FALSE, eval = length(levels(fulldata$Osztály)) > 1}
box_osztály = ggplot(data = fulldata, aes(x = Osztály, y = Százalék_teljes, fill = Osztály)) + geom_boxplot(lwd = 1) + scale_fill_brewer(palette = "BrBG") + theme_dark()
...
```{r warning = FALSE, message = FALSE, echo = FALSE, EVAL = TRUE}
dens1 = ggplot(data = fulldata, aes(x = Százalék_teljes)) + geom_density(fill = "grey", alpha = 0.7) + labs(x = "Vizsgán elért százalék", y = "Sűrűség") + scale_fill_brewer(palette = "BrBG")
...
split_dens = ggplot(data = fulldata_split, aes(x = Százalék_teljes, fill = Évfolyam)) + geom_density(alpha = 0.5) + labs(fill = "Évfolyam") + labs(x = "Vizsgán elért százalék", y = "Sűrűség") + theme(legend.title = element_text("Évfolyam")) + scale_fill_manual(values = c("#117733", "darkorange3"))
...
```{r warning = FALSE, message = FALSE, echo = FALSE, eval = length(levels(fulldata$Tanár)) > 1}
box_tanar = ggplot(data = fulldata, aes(x = Tanár, y = Százalék_teljes, fill = Tanár)) + geom_boxplot(lwd = 1) + labs(x = "Tanár", y = "Írásbeli százalék") + scale_fill_brewer(palette = "BrBG") + theme_dark()
...
box3 = ggplot(data = fulldata, aes(x = Nem, y = Százalék_teljes, fill = Nem)) + geom_boxplot(lwd = 1) + labs(x = "Nem", y = "Elért százalék")+ scale_fill_brewer(palette = "BrBG") + theme_dark()
...
```{r warning = FALSE, message = FALSE, echo = FALSE}
DTdata = fulldata %>% select(Név, Nem, ifelse(length(levels(fulldata$Tanár)) > 1, "Tanár", 0), ifelse(length(levels(fulldata$Osztály)) > 1, "Osztály", 0), ends_with("százalék"), Szóbeli, Százalék_teljes) %>% mutate_if(is.numeric, round, 2)
