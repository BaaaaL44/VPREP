...
colnames(pairwise_osztaly) = text_spec(c("Osztály 1", "Osztály 2", "n1", "n2", "Tesztstatisztika", "df", "p", "p(korr)"), "html", tooltip = TIPS_pair_osztaly)
...
TIPS_pair_tanar = c("Az első vizsgált tanár", "A második vizsgált tanár", "Az első tanár mintaelemszáma", "A második tanár mintaelemszáma", "t-statisztika, a két tanár átlaga közötti különbségnek a két tanár varianciájával való hányadosa", "t-statisztika szabadsági foka", "p-érték, ha 0.05-nél kisebb, az eredmény statisztikailag szignifikánsnak tekinthető", "Korrigált p-érték")
...
colnames(pairwise_tanar) = text_spec(c("Tanár 1", "Tanár 2", "n1", "n2", "Tesztstatisztika", "df", "p", "p(korr)"), "html", tooltip = TIPS_pair_tanar)
...
```{r warning = FALSE, message = FALSE, echo = FALSE}
DTdata = fulldata %>% select(Név, Nem, ifelse(length(levels(fulldata$Tanár)) > 1, "Tanár", 0), ifelse(length(levels(fulldata$Osztály)) > 1, "Osztály", 0), ends_with("százalék"), Szóbeli, Százalék_teljes) %>% mutate_if(is.numeric, round, 3)

datatable(DTdata, rownames = FALSE, filter = "top", colnames = str_replace_all(colnames(DTdata), "_", " "))
```
