...
stats = stats %>%  mutate_if(is.numeric, round, 2)
...
```{r warning = FALSE, message = FALSE, echo = FALSE, eval = length(levels(fulldata$Tanár)) > 1}
rész_desc = fulldata_long %>% group_by(Feladat) %>% summarize(Átlag = mean(Százalék), Medián = median(Százalék), Szórás = sd(Százalék), MAD = mad(Százalék), Min = min(Százalék), Max = max(Százalék))
...
kable(osztaly_desc, digits = 3) %>% kable_styling("striped", full_width = F, position = "center")
```
...
tidy = tidy(ANOVA)
p_val = tidy[1,6]
colnames(tidy) = c("Paraméter", "DF", "Négyzetösszeg", "Négyzetátlag", "F", "p")
tidy[,1] = c("Osztály", "Reziduális")
kable(tidy, digits = 3) %>% kable_styling("striped")
...
tidy = tidy(osztály_t)
p_val = tidy[5]
colnames(tidy) = c("Különbség",levels(fulldata$Osztály)[1],levels(fulldata$Osztály)[2], "t", "p", "df", "Konf.int.alsó", "Konf.int.felső", "módszer", "Alt.")
tidy[9] = "Welch kétmintás t-próba"
tidy[10] = "kétoldalas"
kable(tidy, digits = 3) %>% kable_styling("striped", full_width = F, position = "center")
...
ttest = pairwise.t.test(fulldata$Százalék_teljes, fulldata$Osztály, p.adj = "holm")
tidy_pairwise = tidy(ttest)
colnames(tidy_pairwise) = c("Csoport 1", "Csoport 2", "p")
kable(tidy_pairwise, digits = 3) %>% kable_styling("striped", full_width = F, position = "center")
...
kable(tanar_desc, digits = 3) %>% kable_styling("striped", full_width = F, position = "center")
```
...
colnames(tidyt_t) = c("Különbség",levels(fulldata$Tanár)[1],levels(fulldata$Tanár)[2], "t", "p", "df", "Konf.int.alsó", "konf.int.felső", "módszer", "Alt.")
tidyt_t[9] = "Welch kétmintás t-próba"
tidyt_t[10] = "kétoldalas"
kable(tidyt_t, digits = 3) %>% kable_styling("striped", full_width = F, position = "center")
...
colnames(tidy_tanár) = c("Paraméter", "DF", "Négyzetösszeg", "Négyzetátlag", "F", "p")
tidy_tanár[,1] = c("Tanár", "Reziduális")
kable(tidy_tanár, digits = 3) %>% kable_styling("striped")
```
...
pairwise_tanár = pairwise.t.test(fulldata$Százalék_teljes, fulldata$Tanár, p.adj = "holm")
tidy_pairwise = tidy(pairwise_tanár)
colnames(tidy_pairwise) = c("Csoport 1", "Csoport 2", "p")
kable(tidy_pairwise, digits = 3) %>% kable_styling("striped", full_width = F, position = "center")
```
...
kable(nemek, digits = 3) %>% kable_styling("striped", full_width = F, position = "center")
```
...
tidyt_n = tidy(nem_t)
colnames(tidyt_n) = c("Különbség",levels(fulldata$Nem)[1],levels(fulldata$Nem)[2], "t", "p", "df", "Konf.int.alsó", "Konf.int.felső", "módszer", "Alt.")
tidyt_n[9] = "Welch kétmintás t-próba"
tidyt_n[10] = "kétoldalas"
kable(tidyt_n, digits = 3) %>% kable_styling("striped", full_width = F, position = "center")
...
if (sum(!is.na(fulldata$Szóbeli)) <= 3){
 corrdata = fulldata %>% select(-one_of(c("Név", "Nem", "Tanár", "Osztály", "Százalék_teljes", "megj", "Írásbeli_százalék"))) %>% select(ends_with("százalék"), -Cols_AllMissing(.), -Szóbeli_százalék) 
...
corrdata = fulldata %>% select(-one_of(c("Név", "Nem", "Tanár", "Osztály", "Százalék_teljes", "megj", "Írásbeli_százalék"))) %>% select(ends_with("százalék"), -Cols_AllMissing(.))
corrmat = cor(corrdata)
corrmat = round(corrmat,3)
upper = corrmat
upper[upper.tri(corrmat)]<-""
